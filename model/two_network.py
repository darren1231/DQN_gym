...
def trainNetwork(s,s_for_state, readout,state_value, h_fc1, sess,sess2):
    ...
    writer = tf.train.SummaryWriter("log1/", sess.graph)
    ...
    while "pigs" != "fly":
        ...
        if t > OBSERVE:
            ...
            state_former_batch = sess2.run(state_value,feed_dict = {s_for_state : s_j_batch_for_state})
            state_later_batch = sess2.run(state_value,feed_dict = {s_for_state : s_j1_batch_for_state})
            
                        
            r_hat_batch = []
            for i in range(0, len(minibatch)):
                r_hat_batch.append( r_batch_for_state[i]+GAMMA*state_later_batch[i]-state_former_batch[i])      
            ...
            for i in range(0, len(minibatch)):
                ...
                if minibatch[i][4]:
                    y_batch.append(r_hat_batch[i])
                    y_batch_for_state.append(r_batch_for_state[i])
                else:
                    y_batch.append(r_hat_batch[i] + GAMMA * np.max(readout_j1_batch[i]))
            ...
            sess2.run(train_step_for_state,feed_dict = {
                    ...
                    s_for_state : s_j_batch_for_state})
