...
def createNetwork():
    ...
    a_variable=weight_variable([800,ACTIONS])
    a_bias    =bias_variable([ACTIONS])
    
    v_varibale=weight_variable([800,1])
    v_bias    =bias_variable([1])
    ...
    c_a,c_v=tf.split(3,2,h_conv3)
    stream_a = tf.reshape(c_a,[-1,800])
    stream_v = tf.reshape(c_v,[-1,800])
    
#    advantage=tf.nn.relu(tf.matmul(stream_a,a_variable)+a_bias)
#    value    =tf.nn.relu(tf.matmul(stream_v,v_varibale)+v_bias)
    
    advantage=tf.matmul(stream_a,a_variable)+a_bias
    value    =tf.matmul(stream_v,v_varibale)+v_bias
    
    readout=value+tf.sub(advantage,tf.reduce_mean(advantage,reduction_indices=1,keep_dims=True))
...
def trainNetwork(s, readout, sess,merged,writer):
    ...
    saver.restore(sess, "dueling_networks/pong-dqn-"+str(pretrain_number))
    ...
    while True:
        ...
        if t % 10000 == 0:
            saver.save(sess, 'dueling_networks/' + GAME + '-dqn', global_step = t+pretrain_number)
...
def playGame():
    ...
    writer = tf.train.SummaryWriter("tensorboard_dueling/", sess.graph)
